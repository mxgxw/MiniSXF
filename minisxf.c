/*
   ...
   MiniSXF
   Mini Self eXtract File
   Version 0.1, December 31th, 2012
   This is a sightly modified version of miniunz.c, it's only use is to
   generate a simple self extract executable for Windows. It was
   modified to compile with cygwin so it's possible that it doesn't work
   on other platforms. This is just a demonstration, I cannot guarantee
   that it's going to work on any other platforms.
   
   Once this file its compiled you only need to use "cat" to generate
   the self extract file appending the "payload" (a valid zipfile) at
   the end of the excecutable. This can be done in the command line on
   linux or in a webserver using a PHP script.
   
   Usage example: cat miniunz.exe filename.zip > SelfExtractor.exe
   
   Copyright (C) 2012-1013 Mario Gomez ( http://fuenteabierta.teubi.co )
